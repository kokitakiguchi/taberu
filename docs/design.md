...
> **使用モデル：`claude-haiku-4-5-20251001`（安価モデル）**
> ラベルは「印刷された数値をそのまま読み取る」だけのタスクで、料理写真の推定ほど
> 高い推論力を要しない。AI コスト削減のため dish_photo / text_ai（標準モデル
> `claude-3-5-sonnet-20241022`）とは別に Haiku を使う。Vision の枠組みは維持。
> モデル ID の正本は `backend/src/services/ai.rs` の `MODEL_LABEL` 定数。
